...
# EDGAR resolution
grid.edgar <- data.grid.edgar()
r.transport.edgar <- creainventory::grid.rasterize(emission.transport, grid.edgar)
raster::plot(r.transport.edgar)
saveRDS(r.transport.edgar, "results/r.transport.edgar.RDS")
r.transport.edgar <- readRDS("results/r.transport.edgar.RDS")
